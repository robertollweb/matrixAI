...
def test_generator_declares_index_range_zero_to_vocab_minus_one():
    ...
    assert "categoria: Integer[0, 11]" in gen.mxai_text
    assert "EMBEDDING" in gen.mxai_text and "VOCAB 12" in gen.mxai_text
...
def test_synthetic_dataset_samples_integer_indices():
    ...
    assert all(0 <= s <= 11 for s in samples)
...
def test_embedding_index_not_normalized_in_dataset_csv():
    ...
    assert all(0 <= c <= 9 for c in cats)

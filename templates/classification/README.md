# $project_name — Binary Classification Model
...
Train the model:
```bash
python3 -m matrixai train $project_name.mxai --training $project_name.mxtrain --output runs/v1
```

Make a prediction:
```bash
python3 -m matrixai run $project_name.mxai --params runs/v1/params.best.json --input input/sample.json --json
```

## Files

- `$project_name.mxai` — Model architecture (mathematical function)
- `$project_name.mxtrain` — Training configuration (dataset, hyperparameters)
...
2. Adjust hyperparameters in `$project_name.mxtrain` (learning_rate, epochs, batch_size)
